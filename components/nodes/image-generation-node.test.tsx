...
describe("ImageGenerationNode layout", () => {
  ...
  it("uses a media-first card with working controls and output-attached cost chips", () => {
    ...
    const preview = within(node).getByLabelText("Image generation preview");
    expect(preview).toHaveClass("aspect-square");
  ...
  it("does not render inline advanced settings controls or panel", () => {
    renderNode({
      ...
      history: {
        entries: [
          ...
          },
          {
            ...
            output: { kind: "image", url: "https://fal.media/b.png" },
            actualCost: 0.3,
          },
        ],
        activeId: "b",
      ...
      model: {
        ...
        pricing: { unitPrice: 0.1, unit: "images", currency: "USD" },
      },
...
describe("ImageGenerationNode Pending Output flow (issue #62)", () => {
  const pending = {
  ...
  it("shows a regeneration pending preview and carousel placeholder after FAL accepts the run", async () => {
              ...
              entries: [
                {
                  id: "old",
                  prompt: "a cat",
                  output: { kind: "image", url: "https://fal.media/old.png" },
                },
              ],
              activeId: "old",
            },
    ...
    expect(within(preview).queryByRole("img", { name: /output/i })).not.toBeInTheDocument();
    expect(screen.getByRole("img", { name: /history entry/i })).toHaveAttribute(
      "src",
      "https://fal.media/old.png",
    );
    expect(screen.getByLabelText("Pending history entry")).toBeInTheDocument();

    resolveGeneration({ kind: "image", url: "https://fal.media/new.png" });
